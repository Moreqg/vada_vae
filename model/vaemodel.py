...
class Model(nn.Module):
    ...
    def reparameterize(self, mu, logvar):
        if self.reparameterize_with_noise:
            ...
            eps = torch.cuda.FloatTensor(logvar.size()[0],1).normal_(0,1) # generate a series of numbers, which mean equal 0, stdenv=1
    ...
    def trainstep(self, img, att):
        ...
        ##############################################
        img_from_att = self.decoder['resnet_features'](z_from_att)  # model(input) is same as model.forward()
    ...
    def train_classifier(self, show_plots=False):
        ...
        if self.generalized == False:
            ...
            if self.num_shots > 0:
                ...
                train_unseen_label = self.map_label(train_unseen_label, cls_novelclasses)  # the conculate of matrix and single values
        ...
        with torch.no_grad():
            ...
            # the test_novel_x is regarded as the input of classifier(cls)
            ...
            img_unseen_feat, img_unseen_label = sample_train_data_on_sample_per_class_basis(  # num_shots is 5,
